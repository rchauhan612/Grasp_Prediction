...
def predict_trial(gp_names, gp_list, weight, trial_data):
    ...
    weight = weight[:q]
    time = trial_data[:, 0]
    time = 40*time / time[-1] # remove this when time scaling gets implemented
    traj = np.pi*trial_data[:, 1:q+1]/180. #the pca was done in deg, moving to radians
    for i in range(1, int(.25*l)):
...
for file in os.listdir(trial_dir):
    if file.split('.')[1] == 'npy':
        ...
        trial_data_list.append(np.load(trial_data_loc_list[-1])[:40, :])
